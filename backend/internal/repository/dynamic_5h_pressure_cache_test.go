...
func TestDynamic5hPressureCache(t *testing.T) {
	...
	rollingIDs, err := cache.RollingUserIDs(ctx, now.Add(-5*time.Hour))
	require.NoError(t, err)
	require.Equal(t, []string{"42"}, rollingIDs)
	...
	allowed, pending, err := cache.ReserveUserUsage(ctx, 42, "request-1", 25, 100, 10, now, time.Minute)
	require.NoError(t, err)
	require.True(t, allowed)
	require.InDelta(t, 10, pending, 0.001)
	allowed, _, err = cache.ReserveUserUsage(ctx, 42, "request-2", 95, 100, 10, now, time.Minute)
	require.NoError(t, err)
	require.False(t, allowed)
	require.NoError(t, cache.SettleUserReservation(ctx, 42, "request-1"))
	pending, err = cache.PendingUserUsage(ctx, 42)
	require.NoError(t, err)
	require.Zero(t, pending)
	require.NoError(t, cache.ResetUserUsage(ctx, 42))
	rollingIDs, err = cache.RollingUserIDs(ctx, now.Add(-5*time.Hour))
	require.NoError(t, err)
	require.Empty(t, rollingIDs)
