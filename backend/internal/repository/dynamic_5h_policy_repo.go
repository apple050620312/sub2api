...
type dynamic5hPolicyRepository struct {
	db *sql.DB
}
...
func (r *dynamic5hPolicyRepository) GetUserPolicy(ctx context.Context, userID int64) (service.Dynamic5hUserPolicy, bool, error) {
	...
	if err == sql.ErrNoRows {
		return service.Dynamic5hUserPolicy{Multiplier: 1}, false, nil
	}
...
func (r *dynamic5hPolicyRepository) UpsertUserPolicy(ctx context.Context, userID int64, policy service.Dynamic5hUserPolicy, actorUserID int64, reason string) error {
	...
	old, oldFound, err := dynamic5hPolicyTx(ctx, tx, userID)
	if err != nil {
		return err
	}
	var actor any
...
VALUES ($1,$2,$3,$4,NOW()) ON CONFLICT (user_id) DO UPDATE SET multiplier=EXCLUDED.multiplier, exempt=EXCLUDED.exempt, updated_by=EXCLUDED.updated_by, updated_at=NOW()`, userID, policy.Multiplier, policy.Exempt, actor); err != nil {
		return err
	}
	...
	if _, err = tx.ExecContext(ctx, `INSERT INTO dynamic_5h_audit_logs (user_id, actor_user_id, action, reason, old_policy, new_policy) VALUES ($1,$2,'policy_update',$3,$4,$5)`, userID, actor, reason, oldJSON, string(newJSON)); err != nil {
		return err
	}
	return tx.Commit()
}
...
FROM dynamic_5h_audit_logs a LEFT JOIN users u ON u.id=a.actor_user_id ORDER BY a.created_at DESC LIMIT $1`, limit)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.UserID, &item.ActorUserID, &item.ActorEmail, &item.Action, &item.Reason, &item.BeforeUsage, &item.AfterUsage, &oldJSON, &newJSON, &item.CreatedAt); err != nil {
			return nil, err
		}
		if len(oldJSON) > 0 {
			var p service.Dynamic5hUserPolicy
			if json.Unmarshal(oldJSON, &p) == nil {
				item.OldPolicy = &p
			}
		}
		if len(newJSON) > 0 {
			var p service.Dynamic5hUserPolicy
			if json.Unmarshal(newJSON, &p) == nil {
				item.NewPolicy = &p
			}
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("list dynamic 5h audit logs: %w", err)
	}
	return result, nil
}
